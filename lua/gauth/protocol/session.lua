...
function self:ShouldProcessNotification (groupTreeNode)
	...
	if hostId   == remoteId             then --[[ GAuth.Debug (self:GetType () .. " from " .. remoteId .. ": Session accepted") ]] return true end
	if remoteId == GAuth.GetServerId () then --[[ GAuth.Debug (self:GetType () .. " from " .. remoteId .. ": Session accepted") ]] return true end
